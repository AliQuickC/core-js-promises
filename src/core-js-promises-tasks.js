...
function getAllResult(promises) {
  return Promise.allSettled(promises).then((value) =>
    value.map((item) => (item.status === 'fulfilled' ? item.value : null))
  );
}
...
function queuPromises(promises) {
  return promises.reduce((chain, promiseFunction) => {
    return chain.then((resultString) =>
      promiseFunction.then((result) => resultString + result)
    );
  }, Promise.resolve(''));
}
